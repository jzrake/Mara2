...
// ============================================================================
int BackwardsCompatProgram::run (int argc, const char* argv[])
{
    ...
    if (command == "run")
    {
        ...
        return configuration.launchFromScript (session, argv[1]);
    }
    if (command == "tovtk")
    {
        Logger logger;
    ...
    if (extension == ".lua")
    {
        auto setup = configuration.fromLuaFile (argv[0]);
        session.launch (setup);
        return 0;
    }
    if (extension == ".h5")
    {
        auto setup = configuration.fromCheckpoint (argv[0]);
        session.launch (setup);
        return 0;
    }
